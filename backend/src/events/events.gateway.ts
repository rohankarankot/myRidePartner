...
import {
  buildPublicChatRoomName,
  normalizePublicChatCity,
} from '../public-chat/public-chat-room.util';
...
export class EventsGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  private readonly activeTypers = new Map<
    string,
    Map<number, { userId: number; userName: string }>
  >();
  ...
  async handleConnection(client: Socket) {
    try {
      ...
      if (userId) {
        ...
        client.join(room);
        this.logger.log(
          `Client ${client.id} auto-authenticated as user ${userId}`,
        );
      }
    } catch (err) {
      this.logger.warn(
        `Client ${client.id} failed authentication: ${err.message}`,
      );
    }
  }
  ...
  handleAuthenticate(
    ...
    this.logger.debug(
      `Client ${client.id} called deprecated authenticate event`,
    );
  ...
  handleJoinTrip(
    ...
    const tripDocumentId =
      typeof data === 'string' ? data : data?.tripDocumentId;
    if (tripDocumentId) {
  ...
  async handleJoinChat(
    ...
    const joinedChatRooms =
      (client.data.joinedChatRooms as Set<string> | undefined) ??
      new Set<string>();
  ...
  handleLeaveChat(
    ...
    if (tripDocumentId) {
      ...
      const joinedChatRooms = client.data.joinedChatRooms as
        | Set<string>
        | undefined;
  ...
  async handleJoinPublicChat(
    ...
    if (!city) {
      return {
        status: 'error',
        message: 'Select a city before entering community chat',
      };
    }
    const room = buildPublicChatRoomName(city);
    const joinedPublicChatRooms =
      (client.data.joinedPublicChatRooms as Set<string> | undefined) ??
      new Set<string>();
  ...
  async handleChatLiveLocation(
    ...
    if (!trip || trip.creatorId !== userId) {
      return {
        status: 'error',
        message: 'Only the ride captain can share live location',
      };
    }
  ...
  private emitGroupTypingSnapshot(groupId: string) {
    const typingUsers = Array.from(
      this.activeTypers.get(`group_${groupId}`)?.values() ?? [],
    );
  ...
  private emitTypingSnapshot(tripDocumentId: string) {
    const typingUsers = Array.from(
      this.activeTypers.get(tripDocumentId)?.values() ?? [],
    );
  ...
  private clearTypingForUser(userId?: number) {
    ...
    const keysToClear = Array.from(this.typingTimeouts.keys()).filter((key) =>
      key.endsWith(`:${userId}`),
    );
  ...
  private emitPresenceSnapshot(tripDocumentId: string) {
    const onlineUsers = Array.from(
      this.chatPresence.get(tripDocumentId)?.values() ?? [],
    ).map(({ userId, userName }) => ({
      userId,
      userName,
    }));
  ...
  private setActiveChatViewer(
    ...
    const tripViewers =
      this.activeChatViewers.get(tripDocumentId) ?? new Map<number, number>();
